...
void outfield (double time) {
  ...
  scalar pos[], omega[], omega_air[], p_air[];
  vector tau[];
  ...
  sprintf (fieldname, "./field/field_direct%g", time);
  ...
  foreach(){
    if (interfacial (point, f)){
      ...
      fprintf (feta, "%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g,%g\n", x, pos[], f[], p, p[0,2], p[0,4], tau.x[], tau.x[0,2], tau.x[0,4], tau.y[], u.x[], u.y[], n.x/norm_2, n.y/norm_2);
  ...
  sprintf (matrixname3, "./matrix/tau%g.dat", time);
  FILE * fp1 = fopen (matrixname1, "w");  
  output_matrix (f, fp1, n = 512);
  FILE * fp2 = fopen (matrixname2, "w");  
